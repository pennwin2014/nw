...
function VerticalChart(){
 Graph.call(this);
 this.Margin  = new Array(490,100,300,200);
 this.AxisX  = new Axis();
 this.AxisY  = new Axis();
};
...
_p.drawLineY = function(oContainer){
 ...
 for(var i=0; i<=dcs; i++){
  ...
  newShape= document.createElement("<v:shape style='position:absolute;left:"+ eval(x-420) +";top:"+ eval(py-50) +";WIDTH:400px;HEIGHT:150px;z-index:8' coordsize='21600,21600' fillcolor='white'></v:shape>");
...
_p.drawBar = function(oContainer){
 ...
 for(var i=0; i<iSeriesLn; i++){
  ...
  for(var k=0; k<iValueLn; k++){
   ...
   var mm="<v:rect style='position:absolute;left:"+l+";top:"+t+";WIDTH:"+ barWidth + "px;HEIGHT:"+ barHeight +"px;z-index:9;border-width:0' fillcolor='" + arrSeries[i].Color + "' title = '"+ arrSeries[i].all[k].TooltipText +"'></v:rect>";
