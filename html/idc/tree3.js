...
MzTreeView.prototype.clickHandle = function(e)
{
  ...
  if((e.tagName=="A" || e.tagName=="IMG")&& e.id)
  {
    ...
    if(this.node[id].hasChild){                  //一级树
    	 var temp_src;//&kid@[#idc_sid#]&idc_keyword@[#idc_keyword#]&groupid@[#groupid2#]
		   //temp_src='/pronline/Msg?FunName@ncsIdcRule_maintain&groupid@'+eval(tt).title;
		   
		   temp_src='/pronline/Msg?FunName@ncWebReplace&plate@idc/nc_sub_rule.htm&groupid@'+eval(tt).title;
		   top.sbpm_right.location.href=temp_src;
    }
  else{
//处理点击事件
var temp_src;
  // temp_src='/pronline/Msg?FunName@ncsIdcRule_maintain&kid@'+eval(tt).title;
   ...
   top.sbpm_right.location.href=temp_src;
    ...
    if(eval(tt).title=="rr"){
    	temp_src='/pronline/Msg?FunName@ncWebReplace&plate@idc/nc_sub_rule.htm';
