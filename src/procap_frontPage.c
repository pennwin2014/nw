...
//**************查出离线场所并加入告警表*********************
		...
		//printf("******阈值布控告警***");
//*********************查出每个场所在线人总数，统计count，若超过阈值，warnlog插入数据**************
        char sql[10240] = "";
		...
		//printf("******场所告警恢复***");
//*********************场所告警恢复.统计alarmcode ='10007'告警总数 iSumalarm**************
        int iSumalarm = 0;
...
//*********************设备告警恢复***********************************
			...
			while(psOnline)
			{
				...
				if(strcmp(psOnline->apname,ApTask->caDescription)==0)
				{
					...
					if(time(0) - psOnline->lasttime <= maxTimeoutSeconds)
					{
						...
						//printf("设备离线已经恢复,sql=%s,iRes=%d,isid=%d\n",sql,iRes,isid);
		...
		//printf("******设备告警恢复**释放Ap_alarm_has前。。。。。。。。。。。**");
		 if(Ap_alarm_hash)
            free(Ap_alarm_hash);
		
		//printf("******设备告警恢复**释放Ap_alarm_hash后**");
