...
{  
    ...
    {  
        ...
        lan_MD5Transform(context->state,context->buffer);  
        for(i = partlen;i+64 <= inputlen;i+=64)  
            lan_MD5Transform(context->state,&input[i]);  
        index = 0;          
    }    
    else  
    {  
...
{  
    ...
    lan_MD5Encode(bits,context->count,8);  
    lan_MD5Update(context,PADDING,padlen);  
    lan_MD5Update(context,bits,8);  
    lan_MD5Encode(digest,context->state,16);  
}  
...
int lan_GetMD5(unsigned char * input, int len,  unsigned char * output)
{
	...
	//md5ÑéÖ¤
	unsigned char md5_sid[128];
	unsigned char md5_final[16];

	memset(md5_sid, 0, sizeof(md5_sid)); 
	memset(md5_final, 0, sizeof(md5_final)); 

	MD5_CTX md5;  
   	lan_MD5Init(&md5);            
    	lan_MD5Update(&md5,input, len);  
    	lan_MD5Final(&md5,(unsigned char *)md5_final);
	...
	int n = 0;
	for(n = 0; n < 16; n++){
		snprintf(output+strlen(output), 64-strlen(output), "%02x", (unsigned char)md5_final[n]);
	}
